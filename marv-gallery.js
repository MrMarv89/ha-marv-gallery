 ...
 * - FIX: Race Condition - Added "processing" Set to prevent duplicate work
 * - FIX: Darkness check now actually works for videos
 * - FIX: Timer leak prevention - Single timer instance guaranteed
 * - FIX: Better error handling with proper logging
 * - PERF: O(1) lookups via Map instead of repeated find() calls
 * - PERF: Debounced queue processing
 * - FEATURE: Cache cleanup for old entries (30 days)
 * - FEATURE: Better mobile detection
 */
...
class MarvDB {
  ...
  static async open() {
    ...
    return new Promise((resolve, reject) => {
      ...
      req.onsuccess = () => {
        ...
        resolve(MarvDB._dbInstance);
      };
      
      req.onerror = () => {
        console.error("[MarvDB] Failed to open database:", req.error);
        reject(req.error);
      };
    });
  }
      ...
      return new Promise((resolve) => {
        ...
        req.onsuccess = () => {
          if (req.result) {
            ...
            if (Date.now() - req.result.created > MarvDB.MAX_AGE_MS) {
        ...
        req.onerror = () => {
          console.warn("[MarvDB] Failed to get entry:", id, req.error);
          resolve(null);
        };
      });
    } catch (e) {
      console.warn("[MarvDB] Get error:", e);
      return null;
    }
  }
  ...
  static async put(id, blob, isBroken = false) {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        tx.onerror = () => {
          console.warn("[MarvDB] Failed to put entry:", id, tx.error);
          reject(tx.error);
        };
      });
    } catch (e) {
      console.warn("[MarvDB] Put error:", e);
      return false;
    }
  }
  ...
  static async clear() {
    try {
      ...
      console.warn("[MarvDB] Clear error:", e);
      return false;
    }
  }
  ...
  static async cleanupOldEntries() {
    try {
      const db = await MarvDB.open();
      const cutoff = Date.now() - MarvDB.MAX_AGE_MS;
      ...
      return new Promise((resolve) => {
        ...
        tx.oncomplete = () => {
          if (deletedCount > 0) {
            console.log(`[MarvDB] Cleaned up ${deletedCount} old entries`);
          }
...
class MarvGalleryCard extends LitElement {
  ...
  async _loadMedia(contentId, forceRefresh = false, isSilent = false) {
    ...
    if (!forceRefresh && !contentId && MarvGalleryCard._internalCache.has(path)) {
      const cached = MarvGalleryCard._internalCache.get(path);
    ...
    try {
      ...
      // Update cache
      if (!contentId) {
        MarvGalleryCard._internalCache.set(path, mediaEvents);
      }
    } catch (e) {
      console.error("[MarvGallery] Failed to load media:", e);
      if (!isSilent) {
  ...
  async _fetchRecursive(path) {
    ...
    try {
      ...
      console.warn("[MarvGallery] Failed to fetch recursive:", path, e);
    }
    ...
    return new Promise((resolve) => {
      ...
      const captureFrame = () => {
        try {
          ...
          console.warn("[MarvGallery] Failed to capture frame:", e);
  ...
  async _runWorker(item) {
    ...
    try {
      ...
      console.warn("[MarvGallery] Worker error for:", contentId, e);
      sourceItem.checked = true;
    }
  ...
  async _openPlayer(item) {
    ...
    if (!item.resolved_url) {
      try {
        ...
        console.error("[MarvGallery] Failed to resolve media:", e);
        return;
      }
    }
  ...
  _toggleFullscreen() {
    ...
    if (!document.fullscreenElement) {
      el.requestFullscreen().catch(err => {
        console.warn("[MarvGallery] Fullscreen error:", err);
      });
    } else {
